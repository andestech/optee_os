...
static struct sbi_rpxy sbi_rpxy_hart_data[8];
...
int sbi_rpxy_setup_shmem(unsigned int hartid)
{
	...
	/* Allocate 4-KB memory aligend with 4-KB (required by SBI RPXY). */
	...
	IMSG("Setup RPXY shared memory for hart %d OK, PA: 0x%lX, VA: 0x%lX\n",
